...
public class CoinDatabase {
    ...
    public CoinDatabase() {
        ...
        Collection = new ObservableCollection<PcgsCoin>();
    ...
    public ObservableCollection<PcgsCoin> Collection { get; set; }
    ...
    private SQLiteConnection CreateConnection() {
        ...
        if (!File.Exists("coin_collection")) {
    ...
    /// </summary>
    public void InsertCoin(PcgsCoin pcgsCoin) {
        ...
        cmd.CommandText =
            ...
            $"VALUES ('{pcgsCoin.PcgsNumber}', '{pcgsCoin.Grade}', {pcgsCoin.Year}, '{pcgsCoin.Denomination}', '{pcgsCoin.MintMark}', {pcgsCoin.PriceGuideValue}," +
            $"'{pcgsCoin.CoinFactsLink}', '{pcgsCoin.MajorVariety}', '{pcgsCoin.MinorVariety}', '{pcgsCoin.DieVariety}', '{pcgsCoin.SeriesName}', " +
            $"'{pcgsCoin.Category}', '{pcgsCoin.Designation}', {pcgsCoin.CertificateNumber}, {pcgsCoin.Quantity}, @Notes)";
        cmd.Parameters.AddWithValue("@Notes", pcgsCoin.Notes);
    ...
    public PcgsCoin? GetCoin(int pcgsNumber, string grade) {
        Console.WriteLine($"Getting coin {pcgsNumber}, {grade}...");
        var gradeInt = GetGradeFromString(grade);
        ...
        foreach (var coin in Collection)
            if (coin.PcgsNumber == pcgsNumber.ToString() && coin.Grade.Contains(gradeInt.ToString())) {
                // This has the possibility of a false positive for single digit grades.
                if (gradeInt < 10)
                    if (char.IsDigit(coin.Grade[3]))
                        continue; // TODO For right now, we're going to assume that the first 2 characters are ciruclation strings, and the next 2 are grade numbers.
    ...
    /// </summary>
    /// <param name="collectionName">The name of the collection table.</param>
    /// <param name="pcgsCoin">The coin to save.</param>
    public void SaveCoin(string collectionName, PcgsCoin pcgsCoin) {
        // Get and replace the coin from the database.
        Console.WriteLine("Saving coin...");
        var cmd = Connection.CreateCommand();
        cmd.CommandText = $"UPDATE {collectionName} " +
                          $"SET YEAR = {pcgsCoin.Year}, " +
                          $"DENOMINATION = \'{pcgsCoin.Denomination}\', " +
                          $"MINT_MARK = \'{pcgsCoin.MintMark}\', " +
                          $"PRICE_GUIDE_VALUE = \'{pcgsCoin.PriceGuideValue}\', " +
                          $"COIN_FACTS_LINK = \'{pcgsCoin.CoinFactsLink}\', " +
                          $"MAJOR_VARIETY = \'{pcgsCoin.MajorVariety}\', " +
                          $"MINOR_VARIETY = \'{pcgsCoin.MinorVariety}\', " +
                          $"DIE_VARIETY = \'{pcgsCoin.DieVariety}\', " +
                          $"SERIES_NAME = \'{pcgsCoin.SeriesName}\', " +
                          $"CATEGORY = \'{pcgsCoin.Category}\', " +
                          $"DESIGNATION = \'{pcgsCoin.Designation}\', " +
                          $"CERT_NUM = \'{pcgsCoin.CertificateNumber}\', " +
                          $"QUANTITY = {pcgsCoin.Quantity}, " +
                          $"NOTES = @Notes " +
                          $"WHERE PCGS_NUMBER = {pcgsCoin.PcgsNumber} AND GRADE = \'{pcgsCoin.Grade}\'";
        cmd.Parameters.AddWithValue("@Notes", pcgsCoin.Notes);
    ...
    private PcgsCoin CoinFromReader(SQLiteDataReader reader) {
        // Create a coin while filtering out any possible null values - they throw exceptions.
        PcgsCoin pcgsCoin = new PcgsCoin();
        if (!reader.IsDBNull(0)) pcgsCoin.PcgsNumber = reader.GetString(0);
        if (!reader.IsDBNull(1)) pcgsCoin.Grade = reader.GetString(1);
        if (!reader.IsDBNull(2)) pcgsCoin.Year = reader.GetInt32(2);
        if (!reader.IsDBNull(3)) pcgsCoin.Denomination = reader.GetString(3);
        if (!reader.IsDBNull(4)) pcgsCoin.MintMark = reader.GetString(4);
        if (!reader.IsDBNull(5)) pcgsCoin.PriceGuideValue = reader.GetDouble(5);
        if (!reader.IsDBNull(6)) pcgsCoin.CoinFactsLink = reader.GetString(6);
        if (!reader.IsDBNull(7)) pcgsCoin.MajorVariety = reader.GetString(7);
        if (!reader.IsDBNull(8)) pcgsCoin.MinorVariety = reader.GetString(8);
        if (!reader.IsDBNull(9)) pcgsCoin.DieVariety = reader.GetString(9);
        if (!reader.IsDBNull(10)) pcgsCoin.SeriesName = reader.GetString(10);
        if (!reader.IsDBNull(11)) pcgsCoin.Category = reader.GetString(11);
        if (!reader.IsDBNull(12)) pcgsCoin.Designation = reader.GetString(12);
        if (!reader.IsDBNull(13)) pcgsCoin.CertificateNumber = reader.GetInt32(13);
        if (!reader.IsDBNull(14)) pcgsCoin.Quantity = reader.GetInt32(14);
        if (!reader.IsDBNull(15)) pcgsCoin.Notes = reader.GetString(15);
        // pcgsCoin.TotalPrice = pcgsCoin.Quantity * pcgsCoin.PriceGuideValue;
        return pcgsCoin;
    }
    ...
    /// </summary>
    /// <param name="delPcgsCoin">The coin to delete.</param>
    public void DeleteCoin(PcgsCoin delPcgsCoin) {
        var cmd = Connection.CreateCommand();
        cmd.CommandText = "DELETE FROM CollectionTable WHERE PCGS_NUMBER = @PcgsNumber AND GRADE = @Grade";
        cmd.Parameters.AddWithValue("@PcgsNumber", delPcgsCoin.PcgsNumber);
        cmd.Parameters.AddWithValue("@Grade", delPcgsCoin.Grade);
