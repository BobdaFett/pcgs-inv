...
public partial class PcgsClient : ReactiveObject {
    ...
    protected readonly string _apiUri = "https://api.pcgs.com/publicapi";
    ...
    [Reactive] private int _numApiRequests;
    ...
    public async Task<bool> VerifyCredentials() {
        ...
        var result =
            await GetCoinFactsByGrade(1,
                "AG-03"); // This is a known invalid coin id - this is because the api is very fast with invalid requests.
    ...
    public async Task<(ErrorType, PcgsCoin?)>
        GetCoinFactsByGrade(int pcgsNumber, string grade, bool plusGrade = false) {
    ...
    private int GetGradeFromString(string grade) {
        var actualGrade =
            int.Parse(grade.Substring(grade.Length - 2)); // Throws an exception if the grade doesn't exist.
    ...
    private (ErrorType, PcgsCoin?) ParseJsonResponse(string json) {
        ...
        var newCoin = JsonSerializer.Deserialize<PcgsCoin>(json);
        return (ErrorType.None, newCoin);
    }
}

// Exception for invalid request format
internal class InvalidRequestFormatException : Exception {
